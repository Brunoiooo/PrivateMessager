...
(postgres:17-alpine)   (zależny od db:healthy)   (proxy SSL)   (profil: release)
```
...
API nasłuchuje wewnętrznie na porcie `5000` — nie jest eksponowane bezpośrednio na hosta.
...
Nginx pełni rolę odwrotnego serwera proxy — terminuje TLS (gdy `NGINX_USE_SSL=true`) i przekierowuje ruch do usługi `api:5000`. Certyfikaty SSL umieszcza się w katalogu `./ssl/`.
...
# Zmienne można ustawić z .env lub eksportować ręcznie:
